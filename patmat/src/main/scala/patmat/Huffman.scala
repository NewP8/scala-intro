...
trait Huffman extends HuffmanInterface {
  ...
  def times(chars: List[Char]): List[(Char, Int)] = {
    chars.sorted.foldLeft(List():List[(Char, Int)])((countList, c) => countList match {
      case List() => List()
      case (x,n) :: xs if x == c => (c, n+1) :: xs
      case l => (c,1) :: l
    })
  }
   ...
   */
  def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] = freqs.sortBy(_._2).map(z=>Leaf(z._1, z._2))
  ...
  // print(decodedSecret) List(h, u, f, f, m, a, n, e, s, t, c, o, o, l)
  ...
  def codeBits(table: CodeTable)(char: Char): List[Bit] = {
    table.filter(cl => cl._1 == char).head._2
  }
