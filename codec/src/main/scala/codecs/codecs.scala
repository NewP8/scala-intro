...
trait EncoderInstances {
  ...
  implicit val booleanEncoder: Encoder[Boolean] =
    Encoder.fromFunction(b => Json.Bool(b)) // TODO Define an implicit value of type `Encoder[Boolean]`
...
trait DecoderInstances {
  ...
  implicit def listDecoder[A](implicit decoder: Decoder[A]): Decoder[List[A]] =
    Decoder.fromFunction {
      case Json.Arr(xs) => Some(xs.flatMap(decoder.decode))
      case _ => None
    } //***
...
trait PersonCodecs {
  ...
  implicit val personDecoder: Decoder[Person] =
    Decoder.field[String]("name").zip(Decoder.field[Int]("age")).transform(a =>Person(a._1, a._2))
...
trait ContactsCodecs {
  ...
  implicit val contactsEncoder: Encoder[Contacts] =
    ObjectEncoder.field[List[Person]]("people").transform(c => c.people)

  implicit val contactsDecoder: Decoder[Contacts] =
    Decoder.field[List[Person]]("people").transform(a => Contacts(a))
